...
namespace XCSJ.CommonUtils.PluginCharacters
{
    ...
    /// </summary>
    [Name("角色运动")]
    [Tip("执行所有与角色移动相关的工作(也称为角色发动机）。例如：附加力、冲力、限制(约束)，平台交互等")]
    ...
    public sealed class CharacterMovement : BaseCharacterMB
    {
        ...
        [Name("最大横向速度")]
        [Tip("此角色可以移动的最大横向速度，包括外力的运动，如滑动、碰撞等。")]
        [SerializeField]
        private float _maxLateralSpeed = 10.0f;

        [Name("最大上升速度")]
        [Tip("沿Y轴上移的最大速度，包括外力的运动，如滑动、碰撞等。")]
        [SerializeField]
        private float _maxRiseSpeed = 20.0f;

        [Name("最大下落速度")]
        [Tip("沿Y轴下移的最大速度，包括外力的运动，如滑动、碰撞等。")]
        [SerializeField]
        private float _maxFallSpeed = 20.0f;

        [Name("使用重力")]
        [Tip("启用/禁用角色的自定义重力；如果启用，角色会受到自定义重力的影响。")]
        [SerializeField]
        private bool _useGravity = true;

        [Name("重力")]
        [Tip("此角色的重力大小；通过此字段手动设置重力，来进行更细致的调整和控制。")]
        [SerializeField]
        private float _gravity = 25.0f;

        [Name("陡坡滑坡")]
        [Tip("标识角色是否应该从陡峭的斜坡上滑下来")]
        [SerializeField]
        private bool _slideOnSteepSlope = false;

        [Name("坡度限制")]
        [Tip("可行走斜坡的最大角度（单位:角度）；当坡度大于此值时，角色会滑下斜坡；当坡度小于此值时，角色会停留在斜坡上，不会下滑。")]
        [SerializeField]
        [Range(0, 89)]
        ...
        [Name("捕捉地面")]
        [Tip("启用后，将强制角色安全地跟随可行走的“地面”几何体。")]
        [SerializeField]
        private bool _snapToGround = true;

        [Name("捕捉强度")]
        [Tip("对离“地面”有多近保持角色的容忍度。0不捕捉,1贴地.")]
        [Range(0.0f, 1.0f)]
        [SerializeField]
    ...
    /// 基础角色组件
    /// </summary>
    public abstract class BaseCharacterMB : ToolMB { }
}
