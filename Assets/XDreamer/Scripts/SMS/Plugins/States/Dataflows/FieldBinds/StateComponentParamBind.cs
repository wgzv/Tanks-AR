...
{
    /// <summary>
    /// 状态组件参数绑定:状态组件参数绑定组件是用于将状态组件的属性与一个全局变量绑定在一起，并让属性与变量值保持同步的执行体。状态没退出时，会一直同步属性与变量值。组件在激活之后随即切换为完成态。
    /// </summary>
    [ComponentMenu(DataflowHelper.FieldBinds + "/" + Title, typeof(SMSManager))]
    [Name(Title, nameof(StateComponentParamBind))]
    [Tip("状态组件参数绑定组件是用于将状态组件的属性与一个全局变量绑定在一起，并让属性与变量值保持同步的执行体。状态没退出时，会一直同步属性与变量值。组件在激活之后随即切换为完成态。")]
    [XCSJ.Attributes.Icon(index = 33652)]
    public class StateComponentParamBind : FieldBindOfVirtual<StateComponentParamBind>
    {
        /// <summary>
        /// 标题
        /// </summary>
        public const string Title = "状态组件参数绑定";
